...
class CustomPPOTrainer(PPOTrainer, Trainer):
    ...
    def __init__(
        ...
        #! EDIT : GRPO hyperparams
        self.grpo_num_groups = finetuning_args.grpo_num_groups
        self.grpo_group_size = finetuning_args.grpo_group_size
        self.grpo_alpha = finetuning_args.grpo_alpha
    ...
    def compute_loss(
        self, model: "AutoModelForCausalLMWithValueHead", inputs: Dict[str, "torch.Tensor"], return_outputs: bool = False
    ) -> Union["torch.Tensor", Tuple["torch.Tensor", List["torch.Tensor"]]]:
        r"""
        Computes loss for PPO.
        """
        if self.finetuning_args.use_grpo:
            return self.compute_grpo_loss(model, inputs, return_outputs)
        else:
    ...
    def compute_grpo_loss(
        ...
        group_size = min(self.grpo_group_size, batch_size)
        num_groups = min(self.grpo_num_groups, batch_size // group_size)
        ...
        for i in range(num_groups):
            ...
            group_log_probs = log_probs[start_idx:end_idx]
            group_old_log_probs = old_log_probs[start_idx:end_idx]
            ...
            group_std = group_advantages.std() + 1e-8
            normalized_advantages = (group_advantages - group_mean) / group_std
            
            # Compute policy loss
            surr1 = ratio * normalized_advantages
            surr2 = torch.clamp(ratio, 1.0 - self.finetuning_args.ppo_target, 1.0 + self.finetuning_args.ppo_target) * normalized_advantages
            policy_loss = -torch.min(surr1, surr2).mean()
            
            # Add entropy regularization
            if self.grpo_alpha > 0:
                entropy = -(probs * log_probs).sum(dim=-1).mean()
                policy_loss = policy_loss - self.grpo_alpha * entropy
            
            # Compute value loss
            value_loss = F.mse_loss(values, inputs["returns"])
        ...
        if return_outputs:
            return total_loss, [logits, values]
        return total_loss
