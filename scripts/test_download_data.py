...
def test_dataset_configs():
    ...
    # UltraFeedback should have preference splits
    uf = DATASETS["ultrafeedback"]
    assert "train_prefs" in uf.splits
    assert "test_prefs" in uf.splits
...
def test_ultrafeedback_structure():
    ...
    for split in expected_splits:
        ...
        # UltraFeedback has: prompt, chosen, rejected
        sample = data[0] if data else {}
        
        assert "prompt" in sample or "instruction" in sample, \
            f"Missing prompt/instruction in {split}"
        
        # If it's preference data, should have chosen/rejected
        if "chosen" in sample and "rejected" in sample:
            assert isinstance(sample["chosen"], (str, list)), \
                "chosen should be string or list"
            assert isinstance(sample["rejected"], (str, list)), \
                "rejected should be string or list"
